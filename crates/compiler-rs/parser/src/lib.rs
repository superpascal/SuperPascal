...
impl Parser {
    ...
    /// Parse type: identifier | ^type | ARRAY [ index_type ] OF element_type | RECORD field_list END
    ...
    fn parse_class_type(&mut self) -> ParserResult<Node> {
        let start_span = self
            .current()
            .map(|t| t.span)
            .unwrap_or_else(|| Span::at(0, 1, 1));
    ...
    fn parse_statement(&mut self) -> ParserResult<Node> {
        if self.check(&TokenKind::KwIf) {
            ...
            if is_assignment {
                // Try parsing as assignment - parse_lvalue will handle complex cases
                ...
                if self.check(&TokenKind::Assign) {
                    ...
                    // But we've already consumed tokens, so we need to handle this differently
                    // For now, this shouldn't happen if our check is correct
